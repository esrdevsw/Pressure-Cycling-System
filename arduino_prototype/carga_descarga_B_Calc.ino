...
LiquidCrystal lcd(16, 15, 49, 48, 47, 46);
...
float deltaA = 1;
float deltaB = 1;
float BA = 1; // inclinacao da reta
float BB = 1; // inclinacao da reta
...
void SDcardCin()
{
  ...
  if (dataFileCin)
  {
    ...
    dataFileCin.print("    ");
    dataFileCin.print(BA);
    dataFileCin.print("    ");
    dataFileCin.print(deltaB);
    dataFileCin.print("    ");
    dataFileCin.println(BB);
    dataFileCin.close();
        ...
        if ((estagio==1))// carregar a amostra
        {
         ...
         if (BA >= 0 && BA <= 0.7 &&  estagio==1)
          ...
          if ((nP+1) == nPM && estagio==1)
          {
            ...
            SDcardCin();
            printela();
            Serial.println("abrir valvula para descarga  ");
            digitalWrite(rly03, HIGH);

            for (int ii = 0; ii < taxa; ii++)
            {
              File dataFile = SD.open("datalog.txt", FILE_WRITE);
              dataFile.println(" xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx     ");
              dataFile.println("iniciar descarga da amostra");
              dataFile.print(nP);
              dataFile.print("    ");
              ...
              while (((PvolB/1000)+1) >= 1.03)
              {
                ...
                deltaB = taxa*sigtB2-sigtquad;
                BB =(((taxa*sigtxP)-(sigtB*sigPB))/deltaB)*(-1); // inclinacao da reta





                if (BB >= 0 && BB <= 0.7 && estagio==2)
                {
