...
void Psis()
{
  volA = (analogRead(A3)-60);
  volB = (analogRead(A4)-60);

  // conversao de bits para pressao em milibar

  PvolA = map(volA, -59, 832, 1000, 21207);
  PvolB = map(volB, -59, 832, 1000, 21207);
}
...
void printela()
{
            ...
            Serial.print(defPV01);
            Serial.print("  ");
            Serial.print(PvolA/1000);
            Serial.print("  ");
            Serial.print(temperatura2);
            Serial.print("  ");
            Serial.print(PvolB/1000);
            Serial.print("  ");
            Serial.print("|");
            Serial.print("  ");
            Serial.print(index);
...
void SDcardCin()
{
  ...
  if (dataFileCin)
  {
    ...
    dataFileCin.print("  ");
    dataFileCin.print(PvolA/1000);
    dataFileCin.print("  ");
    dataFileCin.print(temperatura2);
    dataFileCin.print("  ");
    dataFileCin.print(PvolB/1000);
    dataFileCin.print("   ");
...
void loop()
{
  ...
  if (estado == 1) // modo automatico
  {
    ...
    while (ciclos <= Tciclos)
    {


      File dataFile = SD.open("datalog.txt", FILE_WRITE);
      dataFile.println(">>>>>>>>>>>>>>>>>>>>>>>>");
      dataFile.print("lendo o ciclo ");
      dataFile.println(ciclos);

      dataFile.println("index   tempoC  PvolA   ");
      dataFile.close();
      ...
      for (int j = 0; j < 2; j++)
      {
        ...
        for (int i = 0; i < 3; i++)
        {
          ...
          if ((estagio==0))// carga do volume A
          {
            PCrtA();
            SDcardCin();
            ...
            if ((PvolA/1000) > defPV01)
            {
              estagio=1;
              Serial.println("iniciar carga da camara");

              File dataFileCin = SD.open("cinetica.txt", FILE_WRITE);
              dataFileCin.println(">>>>>>>>>>>>>>>>>>>>>>>>");
              dataFileCin.println("iniciar carga da camara");
              dataFileCin.close();
          ...
          if ((estagio==1))// carga da camara
          {
            if (DPDT == 0)
            {
              ...
              {
                estagio=0;
                Serial.println("carregar novamente o volume A");

                File dataFileCin = SD.open("cinetica.txt", FILE_WRITE);
                dataFileCin.println(">>>>>>>>>>>>>>>>>>>>>>>>");
                dataFileCin.println("carregar novamente o volume A");
                dataFileCin.close();
              ...
              else
              {
                ...
                Serial.println("Inicio da descarga");

                File dataFileCin = SD.open("cinetica.txt", FILE_WRITE);
                dataFileCin.println(">>>>>>>>>>>>>>>>>>>>>>>>");
                dataFileCin.println("Inicio da descarga");
                dataFileCin.close();
          ...
          if ((estagio==2))// descarga da camara
          {
            if (DPDT == 0)
            {
              if ((PvolB/1000) > 1.05)
              {
                ...
                {
                  Serial.println(" xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx     ");
                  Serial.println("Inicio do novo ciclo");

                  File dataFileCin = SD.open("cinetica.txt", FILE_WRITE);
                  dataFileCin.println(">>>>>>>>>>>>>>>>>>>>>>>>");
                  dataFileCin.println("Inicio do novo ciclo");
                  dataFileCin.close();
                }
              }
              ...
              {
                Serial.println("limpar volume B");

                File dataFileCin = SD.open("cinetica.txt", FILE_WRITE);
                dataFileCin.println(">>>>>>>>>>>>>>>>>>>>>>>>");
                dataFileCin.println("limpar volume B");
                dataFileCin.close();
        ...
        averageT = totalT / 3;          // calcular a média tempo
        //        Serial.println("========================");
        //        Serial.print("med. t ");
        //        Serial.print(averageT);
        //        Serial.print(" ");
        average = total / 3;          // calcular a média Pressao A
        //        Serial.print("med. P ");
        ...
        //        Serial.println("Dt = taxaT2 - taxaT1");
        //        Serial.print(Dt);
        //        Serial.print(" ");
        //        Serial.print(taxaT2);
        //        Serial.print(" ");
        //        Serial.println(taxaT1);
        //
        //        Serial.println("DP = taxaP2 - taxaP1");
        //        Serial.print(DP);
        //        Serial.print(" ");
        //        Serial.print(taxaP2);
        //        Serial.print(" ");
        //        Serial.println(taxaP1);
        //        Serial.println("");
        //        Serial.println(DPDT);
        //        Serial.println("========================");
